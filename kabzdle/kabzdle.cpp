...
void UpdateScene()
{
	...
	pBackground->Render();
	pBubble->SetRotationCenter(0,0);
	pBubble->SetTranslation(mouseX,mouseY);
	pBubble->Render();

	/*if (rightButton)*/ pBubble->AddRotation(0.05f);

	debugFILE=fopen("debug.txt","at");
	
	fTemp = pBubble->GetDegree(pBubble->GetRotation());
...
//                                         800, 600, LR_LOADFROMFILE );
	...
	pBobul = new CEventEVT("bobul.fox","bobul.evt",pDevice);
		...
		pZzz = new CEventEVT("zzz.fox","zzz.evt",pDevice);
