...
CBitmap::CBitmap(char filename[], char cType)
{
    ...
    unsigned char *pTemp; // wczytane dane
    unsigned char *pTempLine;	// bufor na linie, zeby odwrocic obraz
	...
	pTempLine = new unsigned char [1+(usXSize*4)];	// na linie
    ...
    for (long h1=0; h1<usYSize-1; h1++)
    {
	for (long h2=0; h2<=usXSize; h2++) // najpierw kopiowane dane
	{
	    pTempLine[(h2*4)]=m_pBitmapData[(((usXSize*h1)+h2)*4)];
	    pTempLine[(h2*4)+1]=m_pBitmapData[(((usXSize*h1)+h2)*4)+1];
	    pTempLine[(h2*4)+2]=m_pBitmapData[(((usXSize*h1)+h2)*4)+2];
	    pTempLine[(h2*4)+3]=m_pBitmapData[(((usXSize*h1)+h2)*4)+3];
	}

	for (long h2=0; h2<=usXSize; h2++) // teraz kopiowanie z powrotem od konca
	{
	    m_pBitmapData[(((usXSize*h1)+h2)*4)]= pTempLine[(usXSize*4)-(h2*4)];
	    m_pBitmapData[(((usXSize*h1)+h2)*4)+1]= pTempLine[(usXSize*4)-(h2*4)+1];
	    m_pBitmapData[(((usXSize*h1)+h2)*4)+2]= pTempLine[(usXSize*4)-(h2*4)+2];
	    m_pBitmapData[(((usXSize*h1)+h2)*4)+3]= pTempLine[(usXSize*4)-(h2*4)+3];
	}	
    }
	...
	for (int h1=0; h1<m_ulSizeY; h1++)
	{
		for (int h2=0;h2<m_ulSizeX; h2++)
		{
			if ( ((iX+h2)<1024) && ((iX+h2)>0) &&
			     ((iY+h1)<768) && ((iY+h1)>0) && (m_pBitmapData[(((m_ulSizeX*h1)+h2)*4)+3]>128))
			{
